...
RASDAEMON_OK=0
MSMTP_OK=0
# MSMTP_MTA_OK, msmtp is (or is about to become) the system mail
# transport, which is what makes every sendmail caller on the host a
# reader of /etc/msmtprc. Distinct from MSMTP_OK, which only says the
# binary exists.
MSMTP_MTA_OK=0
...
step1() {
  ...
  if [ "${#need[@]}" -eq 0 ]; then
    ...
    pkg_installed msmtp-mta && MSMTP_MTA_OK=1
    return
  fi
  ...
  pkg_installed rasdaemon && RASDAEMON_OK=1
  { pkg_installed msmtp || pkg_installed msmtp-mta; } && MSMTP_OK=1
  # Recomputed rather than left at the value the plan set: msmtp-mta being
  # wanted is not msmtp-mta being installed, and a partially applied
  # apt-get would otherwise leave this claiming a mail transport that is
  # not there.
  MSMTP_MTA_OK=0
  pkg_installed msmtp-mta && MSMTP_MTA_OK=1
...
OMV_GENERATED_MARKER="auto-generated by openmediavault"

file_is_omv_managed() {
  [ -f "$1" ] && grep -q "$OMV_GENERATED_MARKER" "$1" 2>/dev/null
}

# msmtprc_has_consumer: is there anything on this host that would actually
# read /etc/msmtprc? Exactly two things can:
#
#   - msmtp-mta, which makes msmtp the system mail transport, so every
#     sendmail and mail(1) caller routes through msmtprc. step1 refuses to
#     install it when another package already holds that role, since
#     removing the incumbent is what took OpenMediaVault down with it.
#   - step5's ZED_EMAIL_PROG=msmtp, which names the binary directly and so
#     does not care what the system transport is.
#
# step4 is deliberately NOT on this list. smartd hands mail to sendmail, so
...
confirm_monitoring_change() {
  ...
  local ans
  ans="$(prompt_with_default "$1" "N")"
  case "$ans" in
    [Yy]|[Yy][Ee][Ss]) return 0 ;;
  esac
  CONFIRM_REASON="declined at the prompt (default is No)"
  return 1
}
...
step4() {
  ...
  if [ "$CHECK" -eq 1 ] || [ "$DRY_RUN" -eq 1 ]; then
    note "step4 $file: would ask to configure SMART email alerts and restart smartd (defaults to No; the preview below is what a 'y' would write)"
  fi
...
step5() {
  ...
  if [ "$CHECK" -eq 1 ] || [ "$DRY_RUN" -eq 1 ]; then
    note "step5 $file: would ask to configure ZFS event email and restart zfs-zed (defaults to No; the preview below is what a 'y' would write)"
  fi
...
# Last, deliberately. It can ask a question, and asking it between the
# steps put a prompt in front of step4's, which is the wrong order for an
