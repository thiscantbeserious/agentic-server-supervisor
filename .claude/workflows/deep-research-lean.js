export const meta = {
  ...
  description: 'Token-leane Deep-Research: konsolidierter Haiku-Fetch (mehrere URLs pro Agent), gespreadetes adversariales Verify (unabhängige Voter pro Claim).',
  whenToUse: 'Wie deep-research, aber invertierte Ressourcenverteilung: Fetch gebatcht auf Haiku (mechanische Extraktion), Verify als kleine unabhängige Voter pro Claim (Urteilsqualität braucht Unabhängigkeit, nicht geteilten Kontext).',
  phases: [{"title":"Scope","detail":"Decompose question (from args) into 5 search angles"},{"title":"Search","detail":"5 parallel WebSearch agents, one per angle"},{"title":"Fetch","detail":"URL-dedup, batched: ein Haiku-Agent extrahiert mehrere Quellen","model":"haiku"},{"title":"Verify","detail":"2 unabhängige Voter pro Claim (2/2 refutes to kill)"},{"title":"Synthesize","detail":"Merge semantic dupes, rank by confidence, cite sources"}],
}
...
const VOTES_PER_CLAIM = 2          // 2 unabhängige Voter pro Claim (gespreaded)
const REFUTATIONS_REQUIRED = 2     // beide müssen refuten, um zu killen
const MAX_FETCH = 8                // 8 Quellen
const MAX_VERIFY_CLAIMS = 12       // 12 Claims
const FETCH_BATCH = 4              // ein Haiku-Agent fetcht+extrahiert bis zu 4 Quellen
...
const EXTRACT_SCHEMA = {
  ...
  properties: {
    sources: { type: "array", items: {
      type: "object", required: ["url", "claims", "sourceQuality"],
      properties: {
        url: { type: "string" },
...
const FETCH_PROMPT = (sources, angle) =>
  "## Source Extractor (Batch: " + sources.length + " Quellen)\n\n" +
  "Research question: \"" + QUESTION + "\"\n\n" +
  "Fetch and extract key claims from EACH of these sources (found via " + angle + " search):\n" +
  ...
  "If a fetch fails or a page is irrelevant/paywalled, return that source with claims: [] and sourceQuality: \"unreliable\" — never drop it silently.\n" +
  "Return one entry per source with its exact URL. Structured output only."

const VERIFY_PROMPT = (claim, v) =>
  "## Adversarial Claim Verifier (voter " + (v + 1) + "/" + VOTES_PER_CLAIM + ")\n\n" +
  "Be SKEPTICAL. Try to REFUTE this claim. All " + VOTES_PER_CLAIM + " voters must refute to kill it.\n" +
  "Be token-efficient: 1-2 targeted WebSearch/WebFetch calls, then verdict — no exploratory browsing.\n\n" +
  "## Research question\n" + QUESTION + "\n\n" +
  "## Claim under review\n\"" + claim.claim + "\"\n\n" +
...
const searchResults = await pipeline(
  ...
  searchResult => {
    ...
    // Konsolidierter Fetch: Chunks von FETCH_BATCH Quellen → EIN Haiku-Agent
    // pro Chunk (mechanische Extraktion braucht kein großes Modell). Labels
    // enthalten bewusst KEINEN web-kontrollierten Text (nur Angle + Zählung).
    const chunks = []
    for (let i = 0; i < novel.length; i += FETCH_BATCH) chunks.push(novel.slice(i, i + FETCH_BATCH))
    return parallel(
      chunks.map((chunk, ci) => () =>
        agent(FETCH_PROMPT(chunk, searchResult.angle), {
          label: "fetch:" + searchResult.angle + "#" + ci + " (" + chunk.length + " urls)",
          phase: "Fetch",
          schema: EXTRACT_SCHEMA,
          model: "haiku",
          effort: "low",
        }).then(ext => {
          // User-skip → null; drop the whole chunk (filtered downstream).
          if (!ext) return null
          return chunk.map(source => {
            const e = ext.sources.find(s => normURL(s.url) === normURL(source.url))
            if (!e) return { url: source.url, title: source.title, angle: searchResult.angle, sourceQuality: "unreliable", claims: [] }
            return {
              url: source.url, title: source.title, angle: searchResult.angle,
              sourceQuality: e.sourceQuality, publishDate: e.publishDate,
              claims: e.claims.map(c => ({ ...c, sourceUrl: source.url, sourceQuality: e.sourceQuality })),
            }
          })
        }).catch(err => {
          log("fetch chunk failed (" + searchResult.angle + "#" + ci + ") — " + (err.message || err))
          return chunk.map(source => ({ url: source.url, title: source.title, angle: searchResult.angle, sourceQuality: "unreliable", claims: [] }))
        })
      )
    )
  }
...
phase("Verify")
// Gespreaded: VOTES_PER_CLAIM kleine, UNABHÄNGIGE Voter pro Claim — Urteils-
// qualität kommt aus Unabhängigkeit, nicht aus geteiltem Kontext.
...
return {
  ...
  stats: {
    ...
    agentCalls: 1 + scope.angles.length + Math.ceil(allSources.length / FETCH_BATCH) + (voted.length * VOTES_PER_CLAIM) + 1,
  },
}