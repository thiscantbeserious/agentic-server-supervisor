...
- **T11, host platform support beyond Debian/OMV**, in priority order: **Unraid > TrueNAS SCALE > Arch**.
  ...
  **The platform already has an answer, and writing around it is how a change silently reverts.** OpenMediaVault generates a large part of `/etc` from its own configuration database and marks each generated file:
...
`smartd.conf` and `zed.rc` both carry it, so an installer that writes them appears to succeed, passes every check, and loses the change the next time the platform regenerates, after a settings change, a plugin update, a reconfigure. Disk monitoring stops and the `smart` section goes empty, which reads as healthy rather than as broken. Detecting that marker per file, rather than keeping a list of paths that will be wrong on the next version, is what the installer does now.

  The same is true of notification transport: `postfix` is generated there too, meaning the platform already has a configured mail path and a notification system built on it. Routing `smartd` and ZED through a second, parallel path is what the Debian design does, and it works, but on a platform that already maintains one, using the native route is what the operator has configured and what the platform's own interface reports against. Deciding that per platform, rather than porting the Debian assumption everywhere, is most of this work.
