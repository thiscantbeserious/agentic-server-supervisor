...
stdbuf -oL /usr/bin/gemini --experimental-acp --approval-mode yolo \
  ...
  --allowed-tools="dmesg,df,free,uptime,cat,head,tail,grep,wc,ls,find,journalctl,ss,ip,ping,dig" \
...
# 5. Background reader
{
  while IFS= read -t 600 line <&4; do
    ...
    if [[ -n "$RESP_ID" ]]; then
      RESP_PIPE="$RESP_DIR/$RESP_ID"
      if [[ -p "$RESP_PIPE" ]]; then
        # Open fd once if not already open
        if [[ -z "$OPEN_FD_FOR" || "$OPEN_FD_FOR" != "$RESP_ID" ]]; then
          exec 6>"$RESP_PIPE"
          OPEN_FD_FOR="$RESP_ID"
        fi
        echo "$line" >&6
    ...
    # Streaming updates
    if [[ -n "$CUR_ID" ]]; then
      RESP_PIPE="$RESP_DIR/$CUR_ID"
      if [[ -p "$RESP_PIPE" ]]; then
        if [[ -z "$OPEN_FD_FOR" || "$OPEN_FD_FOR" != "$CUR_ID" ]]; then
          exec 6>"$RESP_PIPE"
          OPEN_FD_FOR="$CUR_ID"
        fi
        echo "$line" >&6
