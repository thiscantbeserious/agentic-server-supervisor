...
7. **Inject keys, meta and resolved.** Drop any model-supplied `key`, `first_seen`, `occurrences` and `meta`; set `f.Key = dedup.Key(f.Component, f.Evidence)` for every finding (D5) and `rep.Meta = &report.Meta{Hostname: cfg.Hostname, TickSeq: o.Seq}` (D9). `info` findings are excluded from the resolve diff: their evidence is model free-text (the quiet-tick finding rephrases freely), so their keys churn tick to tick, and letting them into `resolved[]` pollutes its 20-entry cap and asks `state` to announce that normality was resolved.
