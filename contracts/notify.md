...
1. One absolute deadline, `now + cfg.NotifyTimeout`, set on the dialer **and** on the accepted connection, then `smtp.NewClient` on `net.JoinHostPort(MailriseHost, MailrisePort)`. A cancelled context moves the deadline to now, so shutdown's five-second grace ends a blocked conversation at once. The dial and every read and write of the conversation share that one instant: a dial timeout alone leaves a server that accepts and never greets holding the call (and the outbox drain, and the tick) without limit, and a second deadline started after the dial allows two `NOTIFY_TIMEOUT`s per outbox item where the liveness window (CONTRACTS.md C4) counts one.
...
| 17 | `TestSMTPFallback_HungServerIsBounded`, `TestSMTPFallback_DialAndConversationShareOneDeadline`, `TestSMTPFallback_CancelInterruptsConversation` | a server that accepts TCP and never greets ⇒ `Send(..., smtpFallback=true)` errors within `NOTIFY_TIMEOUT` plus scheduling margin, never hangs; with a dial that itself costs 0.7 × `NOTIFY_TIMEOUT` the call still ends at ~1 × `NOTIFY_TIMEOUT`, not ~1.7 × (one shared deadline, not one per phase); a context cancelled while the conversation is blocked ends the call at once, not at `NOTIFY_TIMEOUT` |
