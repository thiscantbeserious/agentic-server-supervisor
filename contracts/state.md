...
| 13 | liveness | the `heartbeat` file's mtime advances on **every** `Process`, including a fully suppressed one; `Health()` is nil there and non-nil after backdating the mtime past `HealthWindow(cfg)`; `HealthWindow` at the defaults is 2120 s and a heartbeat 1820 s old (the longest legal tick plus the interval) is still healthy; the agy term is `analyze.TriageBudgetTimeouts + 1` hard timeouts (the shared triage budget plus one deep dive), read from `analyze`, not restated |
