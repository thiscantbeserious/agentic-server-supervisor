...
**KVM acceleration is used when the runner offers it, TCG (software emulation) otherwise, and neither job may skip.** `/dev/kvm` on GitHub-hosted runners is present on some runner generations and absent on others, and is explicitly not a supported feature, not a reason to skip: a job that quietly does nothing when its environment is unavailable is the exact failure shape that let `install.sh` go untested this long, and it would be strictly worse here, since this is the one place that failure shape has already cost real data. `test/vm/lib.sh`'s `vm_detect_accel` logs which mode ran and why, so a slow run is explained by its own log rather than left a mystery.
...
**Publishing is isolated to its own job holding `packages: write`; the test job holds only `packages: read`.** `vm-omv-image` is the only place in either variant that can push, it never runs on `pull_request` (`if: github.event_name != 'pull_request'`), so a compromised step reachable from a pull request never has write access to this project's registry namespace. `vm-e2e-omv` (`needs: test`, independent of `vm-omv-image` so it is not skipped when that job is) always tries to pull the content-hash tag first; on a hit it extracts the disk with `docker create`/`docker cp`. On a miss, unpublished (the first run anywhere on this content, most often a pull request that changed the provisioning script before it reached `main`), it builds the image itself and runs the test against it, but never publishes: it has no write scope to do so, and the shared cache is warmed for everyone else only once the change lands on `main` and `vm-omv-image` runs. A cold cache degrades to "this run builds and pays for itself," never to a failure.
