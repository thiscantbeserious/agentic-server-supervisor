...
1. `apt-get install -y --no-install-recommends rasdaemon lm-sensors msmtp msmtp-mta`, skipped per package when `dpkg-query -W -f='${Status}'` already reports installed.
   ...
   **Every `apt-get` call waits for the dpkg lock**: `-o DPkg::Lock::Timeout=300` on the update, the simulated install (`apt-get install -s`, which takes the lock too when run as root, as this script always is) and the real install alike, including `ensure_curl`'s own pair. `unattended-upgrades` runs daily on Debian and OpenMediaVault hosts and holds `/var/lib/dpkg/lock-frontend` for its whole run; apt's default is to fail at once (`E: Could not get lock ... It is held by process N (unattended-upgr)`), which turned a timing coincidence into a whole failed run, on a real host and in the `vm-e2e-omv` gate alike. The value lives once in the script as `APT_LOCK_TIMEOUT`, chosen to outlast a normal unattended-upgrades run so only a genuinely stuck lock exhausts it. A lock still held after the timeout is a real transient failure and keeps its classification: `apt-get install failed`, `exit 75`, safe to re-run. Requires apt 1.9.11 or newer, present on every supported host (Debian bookworm and trixie).
