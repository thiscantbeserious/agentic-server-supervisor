...
services:
  sentinel:
    image: ghcr.io/thiscantbeserious/ai-ops-nanny/sentinel:${SENTINEL_TAG:-latest}
...
4. `docker/metadata-action@v5` → `images: ghcr.io/thiscantbeserious/ai-ops-nanny/sentinel` (a pinned literal, not derived from `github.repository`), tags `type=raw,value=latest,enable={{is_default_branch}}` and `type=sha,format=long,prefix=`
