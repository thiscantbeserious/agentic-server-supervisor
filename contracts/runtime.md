...
**Exit codes** (host script, deliberately its own table — it is not the `sentinel` binary): `0` converged / `--check` clean / `--dry-run` done · `1` `--check` found drift · `64` usage · `69` unsupported host · `70` `systemd-journal` group missing · `75` package install or service restart failed (transient, safe to re-run) · `77` not root · `78` **required ops input missing from `--env-file`** — e.g. `MAILRISE_SMTP_USER`/`MAILRISE_SMTP_PASS` absent, so mail cannot be configured.
