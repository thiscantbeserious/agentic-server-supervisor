...
2. **`omv-confdbadm` (fallback for the empty-root case).** `omv_confdbadm_compose_root` asks OMV's own config database (`omv-confdbadm read conf.service.compose`, resolving the `sharedfolderref` UUID it carries through `conf.system.sharedfolder`) for the plugin's configured path. This is the only signal that can answer the fresh-install, zero-stacks case, and is used exactly there: as the *default proposal* when structural detection finds nothing, and to settle `detect_layout`/the compose-root refusal when the resolved directory's parent has no siblings to pattern-match. It degrades to "unknown" (never to a guess) when `omv-confdbadm` is absent, the plugin is unconfigured, or the output does not parse as expected — this parsing was **not verified against a real OMV host** as part of this change (T8's live validation against `bam` stays read-only per CLAUDE.md); only a live call could prove it matches what a real installation emits. Structural detection is never overridden by this fallback — it only fills the gap structural detection cannot see.
